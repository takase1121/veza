...
test('Create Basic Header', t => {
    ...
    // Receptive header
    {
        const header = Header_1.create(true, new Uint8Array(0));
        ...
        t.equal(Header_1.read32At(header, 7), 0, 'The byte length header must be 0.');
    }
    // Non-receptive header
    {
        const header = Header_1.create(false, new Uint8Array(0));
        t.equal(header.byteLength, 11, 'Headers must always be made of 7 bytes.');
...
test('Write and Read Date', t => {
    ...
    Header_1.writeDate(header, now);
    t.equal(Header_1.readDate(header), now % 0xFFFFFFFF, 'The date bytes must match.');
...
test('Write and Read Increment', t => {
    ...
    const header = new Uint8Array(11);
    Header_1.writeIncrement(header, increment);
    t.equal(Header_1.readIncrement(header), increment, 'The date bytes must match.');
...
test('Create Header From ID', t => {
    t.plan(15);
    const header = Header_1.create(true, new Uint8Array(0));
    const { id, receptive } = Header_1.read(header);
    t.equal(typeof id, 'number', 'The IDs are always numbers.');
    t.equal(receptive, true, 'The header should have receptive as true.');
    const cloned = Header_1.createFromID(id, false, new Uint8Array(0));
    const { id: clonedID, receptive: clonedReceptive } = Header_1.read(cloned);
