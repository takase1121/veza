...
const Header_1 = require("../dist/lib/Util/Header");
...
test('Basic Server', { timeout: 5000 }, async (t) => {
    t.plan(5);
    const nodeServer = new index_1.Server('Server');
    try {
...
test('Basic Socket', { timeout: 5000 }, async (t) => {
    t.plan(16);
    const nodeServer = new index_1.Server('Server');
    const nodeSocket = new index_1.Client('Socket');
    // Open server
    try {
        await nodeServer.listen(++port);
    }
    ...
    try {
        ...
        t.equal(myServer.status, index_1.ClientSocketStatus.Ready, 'The socket should have a status of ready.');
        ...
        t.equal(mySocket.status, index_1.ServerSocketStatus.Connected, 'The socket should have a status of ready.');
...
test('Socket Unknown Server Disconnection (Invalid)', { timeout: 5000 }, async (t) => {
    ...
    try {
        ...
        t.equal(error.message, 'The socket Unknown is not connected to this one.', 'Disconnecting from unconnected sockets should always throw an error.');
    }
});
test('Socket Events', { timeout: 5000 }, async (t) => {
    t.plan(11);
    const nodeServer = new index_1.Server('Server');
    const nodeSocket = new index_1.Client('Socket', { maximumRetries: 0 });
    await nodeServer.listen(++port);
    ...
    nodeSocket.on('connect', client => {
        ...
        t.equal(client.status, index_1.ClientSocketStatus.Connected, 'When this event fires, the status should be "Connected".');
    ...
    nodeSocket.on('ready', client => {
        ...
        t.equal(client.status, index_1.ClientSocketStatus.Ready, 'When this event fires, the status should be "Ready".');
    ...
    nodeSocket.on('disconnect', client => {
        ...
        t.equal(client.status, index_1.ClientSocketStatus.Disconnected, 'When this event fires, the status should be "Disconnected".');
...
test('Client Events', { timeout: 5000 }, async (t) => {
    ...
    nodeServer.on('connect', client => {
        ...
        t.equal(client.status, index_1.ServerSocketStatus.Connected, 'When this event fires, the status should be "Connected".');
        t.equal(client.queue.size, 0, 'The queue must be empty during connection.');
    });
    ...
    nodeServer.on('disconnect', async (client) => {
        ...
        t.equal(client.status, index_1.ServerSocketStatus.Disconnected, 'When this event fires, the status should be "Disconnected".');
        t.equal(client.queue.size, 0, 'The queue must be empty during a disconnection.');
        await nodeServer.close();
    });
...
test('Socket Connection Retries (Successful Reconnect | Different Name)', { timeout: 7500 }, async (t) => {
    ...
    const nodeServerSecond = new index_1.Server('NewServer');
...
test('HTTP Socket', { timeout: 5000 }, async (t) => {
    ...
    try {
        await new Promise((resolve, reject) => {
            http_1.get(new url_1.URL(`http://localhost:${port}`), resolve)
                .on('close', resolve)
                .on('error', reject);
        });
...
test('HTTP Server', { timeout: 5000 }, async (t) => {
    t.plan(5);
    const nodeSocket = new index_1.Client('Socket', { handshakeTimeout: 250 });
    ...
    try {
        ...
        t.true(error instanceof Error, 'The error thrown should be an instance of Error.');
        t.equal(error.message, 'Connection Timed Out.', 'Servers like HTTP ones do not send a message upon connection, so a server that does not send anything is expected to time out.');
    }
    server.close(error => {
...
test('HTTP Server (Incorrect Handshake)', { timeout: 5000 }, async (t) => {
    t.plan(5);
    const nodeSocket = new index_1.Client('Socket', { handshakeTimeout: -1 });
    const server = http_1.createServer(() => {
        t.fail('This should not be called - in Veza, the server sends the message, and the socket replies.');
    });
    server
    ...
    try {
        ...
        t.true(error instanceof Error, 'The error thrown should be an instance of Error.');
        t.equal(error.message, 'Unexpected response from the server.', 'The message sent by the HTTP server is not binaryTF, therefore this should fail.');
    }
    server.close(error => {
        t.equal(error, undefined, 'There should not be an error with closing the server.');
    });
...
test('HTTP Server (Malicious Forged Handshake)', { timeout: 5000 }, async (t) => {
    ...
    server
        ...
        .on('connection', socket => {
        t.pass('A connection should be able to be made.');
        const serialized = v8_1.serialize(420);
...
test('ClientSocket Socket Retrieval', { timeout: 5000 }, async (t) => {
    ...
    try {
        ...
        t.equal(error.message, 'Expected a string or a ClientSocket instance.', 'An invalid NodeServer#get throws a TypeError explaining what was wrong.');
    }
    await nodeServer.close();
    nodeSocket.disconnectFrom('Server');
});
test('NodeServer Socket Retrieval', { timeout: 5000 }, async (t) => {
    ...
    try {
        ...
        t.true(error instanceof TypeError, 'The error should be an instance of TypeError.');
        t.equal(error.message, 'Expected a string or a ServerClient instance.', 'An invalid NodeServer#get throws a TypeError explaining what was wrong.');
    }
    await nodeServer.close();
    nodeSocket.disconnectFrom('Server');
});
test('Socket Message', { timeout: 5000 }, async (t) => {
    ...
    // Test receptive (default) message delivery
    {
        ...
        const response = await nodeSocket.sendTo('Server', 'Hello');
    ...
    // Test non-receptive message delivery
    {
        ...
        const response = await nodeSocket.sendTo('Server', 'Foo', { receptive: false });
...
test('Socket Unknown Server Message Sending (Invalid)', { timeout: 5000 }, async (t) => {
    ...
    try {
        ...
        t.equal(error.message, 'Failed to send to the socket: It is not connected to this client.', 'Sending messages to unconnected sockets should always throw an error.');
    }
});
test('Server Messages', { timeout: 5000 }, async (t) => {
    ...
    try {
        ...
        t.true(error instanceof Error, 'The error should be an instance of Error.');
        t.equal(error.message, 'Failed to send to the socket: It is not connected to this server.', 'Trying to send a message to an unknown socket sends this message.');
    }
    await nodeServer.close();
    nodeSocket.disconnectFrom('Server');
});
test('Server Message (Large Buffer)', { timeout: 5000 }, async (t) => {
    t.plan(5);
    const [nodeServer, nodeSocket] = await setup(t, ++port);
    const buffer = fs_1.readFileSync('./static/logo.png');
    nodeSocket.on('message', message => {
        t.equal(message.receptive, true, 'The message was sent as receptive.');
...
test('Server Message (Multiple Large Buffer)', { timeout: 5000 }, async (t) => {
    t.plan(8);
    const [nodeServer, nodeSocket] = await setup(t, ++port);
    const bufferLogo = fs_1.readFileSync('./static/logo.png');
    const bufferTest = fs_1.readFileSync('./test/test.png');
...
test('Server Message (Spam)', { timeout: 10000 }, async (t) => {
    ...
    nodeSocket.on('message', message => {
        ...
        if (pendingMessages < 0)
            t.fail('Received too many messages');
    });
    try {
        ...
        await Promise.all(numbers.map(n => socket.send(n, { timeout: 10000 }).then(r => {
            if (n !== r)
                t.fail(`Mismatching response. Expected ${n} but received ${r}`);
        })));
    }
    catch (error) {
        t.error(error, 'This should not fail.');
...
test('Socket Faulty Message', { timeout: 5000 }, async (t) => {
    ...
    nodeServer.on('error', async (error, socket) => {
        ...
        t.true(error instanceof Error, 'The error should be an instance of Error.');
        t.equal(error.message, 'Failed to parse message: Unknown type received: 255 [UnknownType]', 'Faulty messages after having connected fire the error event, but does not disconnect.');
        await nodeServer.close();
        nodeSocket.disconnectFrom('Server');
    });
    // Send faulty message
    nodeSocket.get('Server').socket.write(Header_1.create(false, new Uint8Array([0xFF, 0xFF])));
});
test('Server Faulty Message', { timeout: 5000 }, async (t) => {
    ...
    // Send faulty message
    nodeServer.get('Socket').socket.write(Header_1.create(false, new Uint8Array([0xFF, 0xFF])));
});
...
test('Message Broadcast', { timeout: 5000 }, async (t) => {
    ...
    try {
        ...
        t.true(error instanceof TypeError, 'The error should be an instance of TypeError.');
        t.equal(error.message, 'filter must be a RegExp instance.', 'An invalid Node#broadcast filter option throws a TypeError explaining what was wrong.');
    }
    await nodeServer.close();
    nodeSocket.disconnectFrom('Server');
});
...
test('Duplicated Socket', { timeout: 5000 }, async (t) => {
    ...
    const nodeSocketSecond = new index_1.Client('Socket');
...
async function setup(t, port, socketNodeOptions) {
    const nodeServer = new index_1.Server('Server');
    const nodeSocket = new index_1.Client('Socket', socketNodeOptions);
    try {
        // Open server
        await nodeServer.listen(port);
        await nodeSocket.connectTo(port);
